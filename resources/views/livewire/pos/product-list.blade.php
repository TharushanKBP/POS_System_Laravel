            ...
            <div class="row position-relative">
                ...
                <div wire:click.prevent="selectProduct({{ $product }})" class="col-lg-4 col-md-6 col-xl-3" style="cursor: pointer;">
                    <div class="card border-0 shadow h-100">
                        ...
                        <div class="card-body">
                            <div class="mb-2">
                                <h6 style="font-size: 13px;" class="card-title mb-0">{{ $product->product_name }} :- {{ $product->product_note}}</h6>
                                <span class="badge badge-success">
                                    {{ $product->product_code }}
                                </span>
                            </div>
                            <p class="card-text font-weight-bold" style="color: red;">Item Cost :- {{ format_currency($product->product_cost) }}</p>
                            <p class="card-text font-weight-bold">Item Price :- {{ format_currency($product->product_price) }}</p>
                        </div>
                    </div>
                </div>
                @empty
