    ...
    @can('show_notifications')
        <li class="c-header-nav-item dropdown d-md-down-none mr-2">
                ...
                <span class="badge badge-pill badge-danger">
                    @php
                        $low_quantity_products = \Modules\Product\Entities\Product::select(
                            ...
                            ->get();
                        $expired_products = \Modules\Product\Entities\Product::select(
                            'id',
                            'product_code',
                            'product_exp_date',
                        )
                            ->whereMonth('product_exp_date', now()->month)
                            ->whereDay('product_exp_date', '<=', now()->day)
                            ->get();
                        echo $low_quantity_products->count() + $expired_products->count();
                    @endphp
                </span>
            </a>
            <div class="dropdown-menu dropdown-menu-right dropdown-menu-lg pt-0">
                ...
                {{-- Expired Products --}}
                @forelse($expired_products as $product)
                    <a class="dropdown-item text-danger" href="{{ route('products.show', $product->id) }}">
                        <i class="bi bi-exclamation-triangle mr-1 text-danger"></i> Product: {{ $product->product_code }}
                        has expired!
                    </a>
                @empty
                    {{-- No expired products --}}
                ...
                @if ($low_quantity_products->isEmpty() && $expired_products->isEmpty())
                    <a class="dropdown-item" href="#">
                        <i class="bi bi-app-indicator mr-2 text-info"></i> No notifications available.
                    </a>
                @endif
            </div>
        </li>
